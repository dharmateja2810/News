  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      void (async () => {
        setLoading(true);
        try {
          const res = await listArticles({ search: q, limit: 30, page: 1 });
          setCards(res.articles.map(articleToCard));
        } finally {
          setLoading(false);
        }
      })();
...
const styles = StyleSheet.create({
  container: { flex: 1 },
  searchHeader: { paddingHorizontal: 20, paddingTop: 12, paddingBottom: 8 },
